...
inline void require_impl(bool cond, const char *expr, const char *file,
                         int line) {
  if (cond)
    return;
  throw std::runtime_error(
      tf_wrap::detail::format("REQUIRE failed: {} ({}:{})", expr, file, line));
}
...
inline void require_throws_impl(Fn &&fn, const char *expr, const char *ex_name,
  ...
  try {
    ...
    throw std::runtime_error(tf_wrap::detail::format(
        "REQUIRE_THROWS failed: {} did not throw {} ({}:{})", expr, ex_name,
        file, line));
  } catch (const Ex &) {
    // Expected
  } catch (const std::exception &e) {
    throw std::runtime_error(tf_wrap::detail::format(
        "REQUIRE_THROWS failed: {} threw wrong type: {} ({}:{})", expr,
        e.what(), file, line));
  }
}
...
template <class Fn>
inline void require_throws_any_impl(Fn &&fn, const char *expr, const char *file,
                                    int line) {
  try {
    fn();
    throw std::runtime_error(tf_wrap::detail::format(
        "REQUIRE_THROWS failed: {} did not throw ({}:{})", expr, file, line));
  } catch (...) {
...
#define REQUIRE_FALSE(expr)                                                    \
  tf_test::require_impl(!(expr), "!" #expr, __FILE__, __LINE__)
#define REQUIRE_THROWS_AS(expr, ex)                                            \
  tf_test::require_throws_impl<ex>([&] { (void)(expr); }, #expr, #ex,          \
                                   __FILE__, __LINE__)
#define REQUIRE_THROWS(expr)                                                   \
  tf_test::require_throws_any_impl([&] { (void)(expr); }, #expr, __FILE__,     \
                                   __LINE__)
...
TEST_CASE("negative dimension throws") {
  ...
  REQUIRE_THROWS_AS(tf_wrap::FastTensor::Allocate<float>(shape),
...
TEST_CASE("Adopt with null data and zero bytes succeeds") {
  ...
  // Should not throw
  auto tensor =
      tf_wrap::FastTensor::Adopt(TF_FLOAT, shape, nullptr, 0, dealloc);
  REQUIRE(tensor.num_elements() == 0);
}
...
TEST_CASE("GetOperationOrThrow on empty graph throws") {
  ...
  REQUIRE_THROWS_AS(graph.GetOperationOrThrow("nonexistent"),
                    std::runtime_error);
}
...
TEST_CASE("Status all error codes have names") {
  std::vector<TF_Code> codes = {TF_OK,
                                TF_CANCELLED,
                                TF_UNKNOWN,
                                TF_INVALID_ARGUMENT,
...
STRESS_TEST("concurrent tensor creation") {
  ...
  std::cout << "    " << completed.load() << " concurrent ops in " << ms
            << "ms\n";
}
...
STRESS_TEST("fuzz: random tensor shapes") {
  ...
  for (int i = 0; i < 500; ++i) {
    ...
    for (int d = 0; d < rank; ++d) {
      ...
      if (dim == 0) {
        total_elements = 0;
                 ...
                 total_elements <=
                     std::numeric_limits<std::int64_t>::max() / dim) {
        total_elements *= dim;
      }
    }
    ...
    try {
      ...
      REQUIRE(static_cast<std::int64_t>(tensor.num_elements()) ==
              total_elements);
  ...
  std::cout << "    Created " << created << " tensors, " << failed_expected
            << " expected failures\n";
  REQUIRE(created > 200);
}
...
STRESS_TEST("fuzz: moved-from object safety") {
  for (int i = 0; i < 50; ++i) {
    tf_wrap::FastTensor t1 =
        tf_wrap::FastTensor::FromScalar<float>(static_cast<float>(i));
...
TEST_CASE("ImportGraphDef on frozen graph throws") {
  ...
  REQUIRE_THROWS(
      g.ImportGraphDef(graphdef.data(), graphdef.size(), "imported_"));
}
...
TEST_CASE("Session Run with empty fetches") {
  ...
  // Real TensorFlow requires at least one fetch or target and throws
  // INVALID_ARGUMENT. Stub mode returns empty results. Both behaviors are
  // acceptable.
