...
//   session.Run({Feed{"x", input}}, ...);  // If Run tried to lock input →
//   DEADLOCK
//
...
//      then call code that tries to acquire another on the same tensor,
//      deadlock.
//
// SAFE PATTERNS:
//
...
public:
  ...
  Buffer(const void *data, std::size_t length) : buf_(TF_NewBuffer()) {
    ...
    if (data && length > 0) {
      ...
      if (!buf_)
        throw std::runtime_error("TF_NewBufferFromString failed");
    }
  }
...
struct Fetch {
  ...
  Fetch(std::string name, int idx = 0) : op_name(std::move(name)), index(idx) {}
};
...
template <policy::LockPolicy Policy = policy::NoLock> class Session {
public:
  ...
  explicit Session(Graph<GraphPolicy> &graph,
    ...
    if (!graph_handle_) {
      throw std::runtime_error(
          "Session: cannot create session from moved-from graph");
    }
  ...
  template <policy::LockPolicy GraphPolicy>
  explicit Session(Graph<GraphPolicy> &graph, TF_SessionOptions *opts)
      : graph_handle_(graph.handle()) {
    if (!graph_handle_) {
      throw std::runtime_error(
          "Session: cannot create session from moved-from graph");
  ...
  Session(Session &&other) noexcept
      : session_(other.session_), graph_handle_(other.graph_handle_),
  ...
  /// @param run_options Optional serialized RunOptions protobuf for
  /// tracing/timeout
  /// @param run_metadata Optional buffer to receive RunMetadata protobuf
  /// (profiling info)
  [[nodiscard]] std::vector<Tensor<>>
  Run(const std::vector<Feed> &feeds, const std::vector<Fetch> &fetches,
      const std::vector<std::string> &targets, TF_Buffer *run_options,
    ...
    [[maybe_unused]] auto guard =
        policy_.scoped_lock(); // Lock SESSION for entire run
    ...
    for (const auto &f : feeds) {
      TF_Operation *op =
          TF_GraphOperationByName(graph_handle_, f.op_name.c_str());
      if (!op) {
    ...
    for (const auto &t : targets) {
      ...
      if (!op) {
        throw std::runtime_error(
            tf_wrap::detail::format("Target operation '{}' not found", t));
      }
    ...
    TF_SessionRun(session_, run_options,
                  ...
                  static_cast<int>(target_ops.size()), run_metadata, st.get());
  ...
  [[nodiscard]] std::vector<Tensor<>>
  Run(const std::vector<Feed> &feeds, const std::vector<Fetch> &fetches,
      const std::vector<std::string> &targets, const Buffer &run_options,
  ...
  [[nodiscard]] std::vector<Tensor<>>
  RunWithMetadata(const std::vector<Feed> &feeds,
  ...
  [[nodiscard]] std::vector<Tensor<>>
  Run(const std::vector<Fetch> &fetches) const {
    return Run({}, fetches, {});
  }
  ...
  [[nodiscard]] Tensor<> Run(const std::string &fetch_name,
                             int index = 0) const {
  ...
  ///
  /// @param export_dir Path to the SavedModel directory (contains
  /// saved_model.pb)
  ...
  ///
  [[nodiscard]] static std::pair<Session, Graph<Policy>>
  LoadSavedModel(const std::string &export_dir,
    ...
    TF_Session *raw_session = TF_LoadSessionFromSavedModel(
        opts.get(),
        nullptr, // run_options
        export_dir.c_str(), tag_ptrs.data(), static_cast<int>(tag_ptrs.size()),
        graph.handle(),
        nullptr, // meta_graph_def output (we don't need it)
        st.get());

    st.throw_if_error(
        tf_wrap::detail::format("LoadSavedModel('{}')", export_dir));
