...
// Forward declaration
template <policy::LockPolicy Policy> class Graph;
...
template <policy::LockPolicy Policy> class OperationBuilder {
public:
  using guard_type = decltype(std::declval<const Policy &>().scoped_lock());

  /// Construct builder (called by Graph::NewOperation)
  OperationBuilder(TF_Graph *graph, const std::string &op_type,
                   const std::string &name, guard_type guard)
      : graph_(graph), guard_(std::move(guard)),
...
// In real TF, this leaks but there's no public API to clean it up
#ifndef NDEBUG
      std::fprintf(stderr,
                   "[TensorFlowWrap WARNING] OperationBuilder destroyed "
                   "without Finish() - "
  ...
  OperationBuilder(OperationBuilder &&other) noexcept
      : graph_(other.graph_), guard_(std::move(other.guard_)),
        desc_(other.desc_), finished_(other.finished_) {
    other.desc_ = nullptr;
  ...
  OperationBuilder &&SetAttrString(const char *name,
                                   std::string_view value) && {
    return std::move(SetAttrString(name, value));
  }

  /// Set function attribute
  OperationBuilder &SetAttrFuncName(const char *name,
...
template <policy::LockPolicy Policy = policy::NoLock> class Graph {
public:
  ...
  void ImportGraphDef(const void *proto, std::size_t proto_len,
    ...
    [[maybe_unused]] auto guard =
        policy_.scoped_lock(); // Exclusive for mutation
    ...
    if (!opts) {
      ...
      throw std::runtime_error(
          "ImportGraphDef: TF_NewImportGraphDefOptions failed");
    }
  ...
  [[nodiscard]] std::optional<TF_Operation *>
  GetOperation(const std::string &name) const {
  ...
  [[nodiscard]] TF_Operation *
  GetOperationOrThrow(const std::string &name) const {
    auto opt = GetOperation(name);
    if (!opt) {
      throw std::runtime_error(
          tf_wrap::detail::format("Operation '{}' not found in graph", name));
    }
    return *opt;
  }
  ...
  [[nodiscard]] OperationBuilder<Policy>
  NewOperation(const std::string &op_type, const std::string &name) {
  ...
  struct TensorPort {
    ...
    std::string
        op_type;       ///< Type of the operation (e.g., "Placeholder", "Const")
  ...
  /// Get information about operations that have no consumers (typical fetch
  /// points) These are operations whose outputs are not used by any other
  /// operation
  ...
  [[nodiscard]] std::vector<TF_Operation *>
  GetOperationsByType(std::string_view op_type) const {
